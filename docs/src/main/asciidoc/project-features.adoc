...
We're providing an out-of-the-box integration with the OTel Jaeger exporter. via the `io.opentelemetry:opentelemetry-exporter-otlp` dependency.
...
OpenTelemetry provides a `Resource` abstraction which captures identifying information about the entities for which signals (stats or traces) are reported. If you wish to provide your own, you can register beans of `ResourceProvider` type.

You can disable the registration of the default `ResourceProvider` beans via the `spring.sleuth.otel.resource.enabled=false` property.
